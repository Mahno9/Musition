...
const MODELS = {
  'stable-audio-open': {
    ...
    about: `
      <p><b>Звуковые эффекты и окружение по тексту, 44.1 kHz стерео</b> — плюс короткие инструментальные куски
      до 47 секунд.</p>
      <ul>
  ...
  'demucs': {
    label: 'Demucs (стемы)',
    about: `
      <p><b>Не генерирует, а разбирает готовый трек на дорожки:</b> вокал, барабаны, бас
      и всё остальное. Работает с любым файлом — своим рендером, чужой музыкой, записью.</p>
      <ul>
        <li><b>Берите его,</b> когда нужен чистый голос отдельной дорожкой, чтобы подмешивать
            его под музыку с управляемым уровнем, или наоборот минус без вокала.</li>
        <li>Просить у генератора «а капелла» бесполезно — модель всё равно подложит
            инструменты. Вокал вырезается уже из готового рендера, здесь.</li>
        <li>Скорость: ~1 минута на двухминутный трек на CPU, на GPU заметно быстрее.
            Веса лёгкие (~80 MB), качается один раз.</li>
        <li>MIT — веса htdemucs обучены на MUSDB18-HQ, использование не ограничено.</li>
      </ul>`,
    main: [
      F('src_audio_path', 'Исходный трек', 'Файл, который надо разделить (он загрузится на сервер)'),
      S('stem', 'Что вернуть', 'both', ['both', 'vocals', 'no_vocals', 'all'],
        'both — голос и минус двумя файлами; all — все источники модели по отдельности'),
    ],
    groups: [
      ['Качество и скорость', [
        S('model_name', 'model_name', 'htdemucs',
          ['htdemucs', 'htdemucs_ft', 'htdemucs_6s', 'hdemucs_mmi', 'mdx_extra', 'mdx_extra_q'],
          'htdemucs_ft — те же 4 источника, но вчетверо дольше; htdemucs_6s добавляет ' +
          'пианино и гитару. Новые веса качаются при первом использовании'),
        N('shifts', 'shifts', 0, 0, 10, 1,
          '0 — один проход, результат воспроизводим. >1 усредняет столько случайных сдвигов ' +
          'и во столько же раз дольше'),
        N('overlap', 'overlap', 0.25, 0, 0.9, 0.05, 'Перекрытие соседних кусков'),
        S('device', 'device', 'auto', ['auto', 'cuda', 'cpu'],
          'auto — cuda, если она доступна. На CPU тоже работает, просто медленнее'),
        N('jobs', 'jobs', 0, 0, 16, 1, 'Параллельные процессы на CPU; 0 — без параллелизма'),
      ]],
    ],
  },
};
...
function boot() {
  ...
  show(MODELS[start] || LINKS[start] || start === 'gallery' ? start : 'stable-audio-open');
  refreshStatus();
